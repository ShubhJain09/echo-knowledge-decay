...
export const comparisonKey = (org: string, cardId: string, version: number, hash: string) =>
  createHash('sha256')
    ...
    .digest('hex');
function activeCard(data: Data, id: string) {
  const c = data.cards.find(c => c.id === id && c.status === 'verified');
  if (!c) throw new AppError(404, 'Choose an active knowledge card.');
  return c;
}
export async function dispatch(
  ...
  const parts = pathname
    ...
    .split('/')
    .filter(Boolean);
  if (method === 'GET' && parts.join('/') === 'workspace') {
  ...
  if (method === 'POST' && parts.join('/') === 'seed') {
    ...
    await repo.mutate(actor, 'knowledge.seed', 'service-x', d => {
      for (const c of cards) if (!d.cards.some(old => old.id === c.id)) d.cards.push(c);
    });
    return { body: { ok: true } };
  }
  if (method === 'POST' && parts.join('/') === 'knowledge') {
    ...
    await repo.mutate(actor, 'knowledge.created', card.id, d => d.cards.push(card));
  ...
  if (method === 'POST' && parts.join('/') === 'evidence') {
    ...
    if (input.encoding === 'base64' && !/^(?:[A-Za-z0-9+/]{4})*(?:[A-Za-z0-9+/]{2}==|[A-Za-z0-9+/]{3}=)?$/.test(input.content))
      throw new AppError(400, 'The file encoding is invalid.');
    ...
    if (/\.pdf$/i.test(input.name)) {
      ...
      try {
        ...
        extracted = (await pdf(new Uint8Array(bytes) as unknown as Buffer, { version: 'v2.0.550' })).text;
      } catch {
    ...
    try {
      ...
      if (error instanceof AppError && error.status === 409) {
        const e = (await repo.snapshot()).evidence.find(e => e.contentHash === contentHash && e.cardId === input.cardId);
        if (e) return { body: { ...e, duplicate: true, message: 'Already ingested.' } };
      }
      throw error;
    }
  ...
  if (parts[0] === 'evidence' && parts[1]) {
    const data = await repo.snapshot();
    const evidence = data.evidence.find(e => e.id === parts[1]);
    ...
    if (method === 'POST' && parts.length === 3 && parts[2] === 'compare') {
      ...
      const review: Review = {
        ...
        priority: ['contradiction', 'process_change', 'dependency_change'].includes(comparison.relation) ? 'high' : 'medium',
      };
      try {
        await repo.mutate(actor, 'review.created', id, d => {
          if (activeCard(d, card.id).version !== card.version) throw conflict();
          if (d.reviews.some(r => r.id === id)) throw conflict();
          d.reviews.push(review);
        });
      } catch (error) {
        if (error instanceof AppError && error.status === 409) {
          const r = (await repo.snapshot()).reviews.find(r => r.id === id);
          if (r) return { body: r };
        }
        throw error;
      }
  ...
  if (method === 'POST' && parts[0] === 'reviews' && parts[2] === 'manage' && parts.length === 3) {
    ...
    if (
      input.snoozedUntil &&
      (Date.parse(input.snoozedUntil) <= Date.now() || Date.parse(input.snoozedUntil) > Date.now() + 90 * 86400000)
    )
      throw new AppError(400, 'Snooze for a future date within 90 days.');
    await repo.mutate(actor, 'review.managed', parts[1], d => {
      const r = d.reviews.find(r => r.id === parts[1] && r.status === 'pending');
      if (!r) throw new AppError(404, 'Pending review not found.');
  ...
  if (method === 'POST' && parts[0] === 'knowledge' && parts[2] === 'rollback' && parts.length === 3) {
    ...
    const input = valid(
      z
        .object({ expectedVersion: z.number().int().positive(), version: z.number().int().positive(), reason: text(1000) })
        .strict(),
      body,
    );
    const result = await repo.mutate(actor, 'knowledge.rollback', parts[1], d => {
      const c = d.cards.find(c => c.id === parts[1]);
      if (!c) throw new AppError(404, 'Knowledge not found.');
      if (c.version !== input.expectedVersion) throw conflict();
      const target = c.history.find(v => v.version === input.version);
      ...
      c.version++;
      c.statement = target.statement;
      c.source = target.source;
      c.status = 'verified';
      c.updatedAt = now;
      c.lastVerifiedAt = now;
      c.nextReviewAt = nextReview(now, c.volatility);
      c.history.push({
        ...target,
        status: 'verified',
        version: c.version,
      ...
      for (const c of d.cards.filter(
        ...
        if (!d.reviews.some(r => r.id === id))
          d.reviews.push({
            ...
            oldQuote: c.statement,
            newQuote: '',
            proposedStatement: c.statement,
            engine: 'policy',
          });
      }
    });
