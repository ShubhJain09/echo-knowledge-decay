...
export interface Store {
  ...
  list<T>(pk: string, prefix?: string): Promise<Row<T>[]>;
...
export class SqliteStore implements Store {
  ...
  constructor(url = process.env.DATABASE_URL || 'file:./echo.db') {
    ...
    this.db =
      url === process.env.DATABASE_URL && globalDb.echoPrisma
        ? globalDb.echoPrisma
        : new PrismaClient({ datasources: { db: { url } } });
    if (url === process.env.DATABASE_URL) globalDb.echoPrisma = this.db;
  }
  async get<T>(pk: string, sk: string) {
    const row = await this.db.record.findUnique({ where: { pk_sk: { pk, sk } } });
    return row ? { ...row, data: JSON.parse(row.data) as T } : undefined;
  }
  async list<T>(pk: string, prefix = '') {
    return (await this.db.record.findMany({ where: { pk, sk: { startsWith: prefix } } })).map(row => ({
      ...row,
...
export class DynamoStore implements Store {
  constructor(
    private table = process.env.KNOWLEDGE_TABLE!,
    private db = DynamoDBDocumentClient.from(new DynamoDBClient({}), { marshallOptions: { removeUndefinedValues: true } }),
  ) {
    if (!table) throw new AppError(503, 'KNOWLEDGE_TABLE is not configured.');
  }
  async get<T>(pk: string, sk: string) {
    const result = await this.db.send(new GetCommand({ TableName: this.table, Key: { pk, sk }, ConsistentRead: true }));
    return result.Item as Row<T> | undefined;
  }
  async list<T>(pk: string, prefix = '') {
    const rows: Row<T>[] = [];
    ...
    do {
      ...
      rows.push(...((result.Items || []) as Row<T>[]));
  ...
  async write(writes: Write[]) {
    ...
    if (writes.length > 100 || writes.some(w => Buffer.byteLength(JSON.stringify(w.row)) > 350000))
      throw new AppError(422, 'Transaction exceeds storage limits. Contact your administrator.');
    try {
      await this.db.send(
        new TransactWriteCommand({
          TransactItems: writes.map(({ row, expected }) => ({
            Put: {
              TableName: this.table,
              Item: row,
              ConditionExpression: expected < 0 ? 'attribute_not_exists(pk)' : 'revision = :revision',
              ...(expected < 0 ? {} : { ExpressionAttributeValues: { ':revision': expected } }),
            },
          })),
        }),
      );
    } catch (error) {
      if ((error as Error).name === 'TransactionCanceledException') throw conflict();
      throw error;
    }
  }
}
...
export function store(): Store {
  return (singleton ||= process.env.ECHO_STORAGE === 'aws' ? new DynamoStore() : new SqliteStore());
}
