...
export const authOptions: NextAuthOptions = {
  ...
  providers: [
    CredentialsProvider({
      ...
      async authorize(credentials) {
        ...
        if (
          !row ||
          row.data.identityProvider !== 'local' ||
          row.data.status !== 'active' ||
          !verifyPassword(input.data.password, row.data.passwordHash || '')
        )
          return null;
        await saveUser(row, { ...row.data, lastSeenAt: new Date().toISOString() });
        await auditAccount(row.data, 'auth.login');
  ...
  callbacks: {
    async signIn({ user, account: provider, profile }) {
      ...
      if (!row) {
        await createAccount({
          ...
          email: user.email,
          organizationName: `${user.name || 'My'} workspace`,
          provider: 'cognito',
          subject: p.sub,
        });
        row = await account(user.email);
      }
    ...
    async jwt({ token, user }) {
      if (user?.email) {
        const row = await account(user.email);
        token.email = user.email;
        token.sessionVersion = row?.data.sessionVersion;
    ...
    async session({ session, token }) {
      if (session.user) {
        (session as unknown as { sessionVersion: unknown }).sessionVersion = token.sessionVersion;
        (session as unknown as { authenticatedAt: unknown }).authenticatedAt = token.authenticatedAt;
      }
      return session;
    },
  },
