...
export async function groq(system: string, input: unknown): Promise<string> {
  ...
  const response = await fetch('https://api.groq.com/openai/v1/chat/completions', {
    ...
    body: JSON.stringify({ model: process.env.GROQ_MODEL_ID || 'openai/gpt-oss-20b', temperature: 0,
      max_completion_tokens: 3000, response_format: { type: 'json_object' },
      messages: [{ role: 'system', content: system }, { role: 'user', content: JSON.stringify({ untrustedData: input }) }] }),
  });
  if (!response.ok) throw new AppError(502, 'Groq inference failed. Please retry.');
  const parsed = z.object({ choices: z.array(z.object({ message: z.object({ content: z.string() }) })).min(1) }).safeParse(await response.json());
