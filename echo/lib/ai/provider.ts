...
export const comparisonSchema = z
  .object({
    ...
    oldQuote: z.string().min(1).max(8000),
    newQuote: z.string().min(1).max(8000),
    proposedStatement: z.string().max(8000).nullable(),
  })
  .strict();
export type CompareInput = {
  knowledge: { statement: string; title: string; topic: string };
  evidence: { text: string; sourceName: string };
...
export class ModelValidationError extends AppError {
  ...
  constructor() {
    super(502, 'The model returned a quote that could not be verified against the sources. Retry the comparison.');
  }
}
export function validateComparison(raw: string, input: CompareInput) {
  ...
  if (!input.knowledge.statement.includes(result.oldQuote) || !input.evidence.text.includes(result.newQuote))
    throw new ModelValidationError();
...
export async function compareKnowledge(input: CompareInput, model: Model = input.provider === 'groq' ? groq : bedrock) {
  for (let attempt = 0; attempt < 2; attempt++) {
    const raw = await model(
      comparisonPrompt +
        (attempt
          ? '\nRETRY: Return only strict JSON. Copy exact source substrings; do not normalize punctuation, spacing or quotes.'
          : ''),
      input,
    );
    try {
      return validateComparison(raw, input);
    } catch (error) {
      if (!(error instanceof ModelValidationError) || attempt === 1) throw error;
    }
  }
  throw new ModelValidationError();
}
