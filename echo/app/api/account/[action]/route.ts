...
export async function POST(request: NextRequest, { params }: { params: Promise<{ action: string }> }) {
  try {
    ...
    if (action === 'verify') {
      const input = valid(z.object({ email: z.email(), token: text(100) }).strict(), body);
      await rateLimit(`verify:${input.email}`, 10, 900);
      const row = await account(input.email);
      if (
        !row ||
        row.data.status !== 'pending' ||
        !row.data.verificationHash ||
        !timingSafeEqual(Buffer.from(row.data.verificationHash), Buffer.from(hash(input.token))) ||
        Date.parse(row.data.verificationExpires || '') < Date.now()
      )
        throw new AppError(400, 'This verification link is invalid or expired.');
      await saveUser(row, { ...row.data, status: 'active', verificationHash: undefined, verificationExpires: undefined });
      await auditAccount(row.data, 'auth.verified');
      return NextResponse.json({ ok: true });
    }
    if (action === 'resend') {
      const input = valid(z.object({ email: z.email() }).strict(), body);
      await rateLimit(`resend:${input.email}`, 3, 3600);
      const row = await account(input.email);
      if (row?.data.status === 'pending') {
        ...
        await sendVerification(input.email, code);
      }
      return NextResponse.json({ ok: true });
    }
    ...
    if (action === 'profile') {
      ...
      try {
        ...
        throw new AppError(400, 'Enter a valid timezone.');
      }
      await saveUser(row, { ...row.data, ...input });
      await auditAccount(row.data, 'account.profile');
      return NextResponse.json(publicUser({ ...row.data, ...input }));
    }
    if (action === 'revoke-sessions') {
      valid(z.object({}).strict(), body);
      await requireRecentLogin();
      await saveUser(row, { ...row.data, sessionVersion: row.data.sessionVersion + 1 });
      await auditAccount(row.data, 'auth.revoke_all');
      return NextResponse.json({ ok: true });
    }
    if (action === 'password') {
      await requireRecentLogin();
      const input = valid(
        z.object({ currentPassword: z.string().max(128), password: z.string().min(12).max(128) }).strict(),
        body,
      );
      if (row.data.identityProvider !== 'local' || !verifyPassword(input.currentPassword, row.data.passwordHash || ''))
        throw new AppError(400, 'Current password is incorrect.');
      await saveUser(row, {
        ...row.data,
        passwordHash: passwordHash(input.password),
        sessionVersion: row.data.sessionVersion + 1,
      });
      await auditAccount(row.data, 'auth.password');
      return NextResponse.json({ ok: true });
    }
    throw new AppError(404, 'Account action not found.');
  } catch (e) {
    return NextResponse.json(
      { error: e instanceof AppError ? e.message : 'Account request failed.' },
